...
#define TMR_AUTOREST_RESTEDRECOIL 0.61
#define TMR_AUTOREST_DEPLOYEDRECOIL 0.495
...
tmr_autorest_fnc_deployKeyDownEH = {
	...
	if (_dikCode in actionKeys "LockTargets") then {
		...
		_canDeploy = getNumber (_config >> "tmr_autorest_deployable"); // 1 for true
		if (_canDeploy != 1) exitwith {false};
...
tmr_autorest_fnc_attachRefPoints = {
	...
	tmr_autorest_refPoint8 attachTo [player, [0,0.24,-0.4], "RightHand"]; 
	tmr_autorest_refPoint9 attachTo [player, [0,0.57,-0.4], "RightHand"]; 
};
