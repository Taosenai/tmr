...
if (!_isFalling) then {
	...
	_unit setVariable ["tmr_falling", true, true];
	...
	if (_detectStance > 0.65) then {
		...
		_unit spawn {
			...
			// Pick a random sound and format the filename
			_sfx = format ["A3\sounds_f\characters\crawl\%1.wss", ["crawl_dirt_1","crawl_dirt_2","crawl_dirt_3","crawl_dirt_4","crawl_dirt_5","crawl_dirt_6","crawl_dirt_7"] call BIS_fnc_selectRandom];

			playSound3D [_sfx, _this, false, getPosASL _this, 1, 1, 20];
		};
